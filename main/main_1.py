...
def to_top():
    ...
    def dd(path, dest):
        ...
        shutil.rmtree(path)
    path = r'E:\FPAN\somepy\mypyfile\test_PIL\shixin_court_gov_cn\images\template_images'
    for root, dirs, files in os.walk(path):
        for dir_c in dirs:
...
def load_template():
    ...
    path = os.path.join(curpath, 'template', '1')
    for num in range(10):
...
def verifyall():
    ...
    path = r'E:\FPAN\somepy\mypyfile\test_PIL\shixin_court_gov_cn\images\original_images'
    for image_name in os.listdir(path):
...
if __name__ == "__main__":
    ...
    #测试单个图片
    img = Image.open('./test_iamge/1.jpg')
